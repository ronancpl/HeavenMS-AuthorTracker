...
result_type = AuthorGraphResultFile.ResultFile.CONTENT_FILE_SIZE_GROWTH
...
def plotfy(data, csv_filename):
    ...
    fig = sns.catplot(x='timeset', y='count', col='filetype', kind='bar', data=data, height=5, aspect=1)
...
def plot():
    ...
    for graph_content_range_idx in range(len(graph_content)):
        ...
        for filetype_graph_content_idx in range(len(graph_content_range)):
